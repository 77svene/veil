...
from veil.dom.views import NodeType, SimplifiedNode
from veil.llm.messages import ContentPartImageParam, ContentPartTextParam, ImageURL, SystemMessage, UserMessage
from veil.observability import observe_debug
from veil.utils import is_new_tab_page, sanitize_surrogates

if TYPE_CHECKING:
	from veil.agent.views import AgentStepInfo
...
class SystemPrompt:
	def __init__(
		...
		self.is_veil_model = is_veil_model
	...
	def _load_prompt_template(self) -> None:
		...
		try:
			...
			if self.is_veil_model:
				if self.flash_mode:
					template_filename = 'system_prompt_veil_flash.md'
				elif self.use_thinking:
					template_filename = 'system_prompt_veil.md'
				else:
					template_filename = 'system_prompt_veil_no_thinking.md'
			...
			with (
				importlib.resources.files('veil.agent.system_prompts')
