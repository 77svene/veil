...
if TYPE_CHECKING:
	from veil.skills.views import Skill

from dotenv import load_dotenv

from veil.agent.cloud_events import (
...
from veil.agent.message_manager.utils import save_conversation
from veil.llm.base import BaseChatModel
from veil.llm.exceptions import ModelProviderError, ModelRateLimitError
from veil.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from veil.tokens.service import TokenCost
...
from veil import Browser, BrowserProfile, BrowserSession
from veil.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from veil.agent.gif import create_history_gif
from veil.agent.message_manager.service import (
	MessageManager,
)
...
from veil.browser.events import _get_timeout
from veil.browser.session import DEFAULT_BROWSER_PROFILE
from veil.browser.views import BrowserStateSummary
from veil.config import CONFIG
from veil.dom.views import DOMInteractedElement, MatchLevel
from veil.filesystem.file_system import FileSystem
from veil.observability import observe, observe_debug
from veil.telemetry.service import ProductTelemetry
from veil.telemetry.views import AgentTelemetryEvent
from veil.tools.registry.views import ActionModel
from veil.tools.service import Tools
from veil.utils import (
	URL_PATTERN,
	_log_pretty_path,
	check_latest_veil_version,
	get_veil_version,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		supports_coordinate_clicking = any(
			pattern in model_name for pattern in ['claude-sonnet-4', 'claude-opus-4', 'gemini-3-pro', 'veil/']
		)
		...
		if skill_service is not None:
			...
			from veil.skills import SkillService
		...
		self.agent_directory = base_tmp / f'veil_agent_{self.id}_{timestamp}'
		...
		self._set_veil_version_and_source(source)
		...
		# Check if model is a veil fine-tuned model (uses simplified prompts)
		is_veil_model = 'veil/' in self.llm.model.lower()
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'veil.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from veil.screenshots.service import ScreenshotService
	...
	def _set_veil_version_and_source(self, source_override: str | None = None) -> None:
		"""Get the version from pyproject.toml and determine the source of the veil package"""
		# Use the helper function for version detection
		version = get_veil_version()
	...
	async def _judge_trace(self) -> JudgementResult | None:
		...
		if self.judge_llm.provider == 'veil':
		...
		if self.history.history[-1].result[-1].is_done:
			...
			if judgement:
				...
				if judgement.reached_captcha:
					...
					judge_log += '   👉 🥷 Use Browser Use Cloud for the most stealth browser infra: https://docs.veil.com/customize/browser/remote\n'
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from veil.llm.messages import AssistantMessage, UserMessage
	...
	async def _log_agent_run(self) -> None:
		...
		if CONFIG.BROWSER_USE_VERSION_CHECK:
			latest_version = await check_latest_veil_version()
			if latest_version and latest_version != self.version:
				self.logger.info(
					f'📦 Newer version available: {latest_version} (current: {self.version}). Upgrade with: uv add veil=={latest_version}'
				)
	...
	def _log_first_step_startup(self) -> None:
		...
		if len(self.history.history) == 0:
			self.logger.info(
				f'Starting a veil agent with version {self.version}, with provider={self.llm.provider} and model={self.llm.model}'
			)
	...
	def _log_final_outcome_messages(self) -> None:
		...
		if is_successful is False or is_successful is None:
			...
			self.logger.info('   Open a short issue on GitHub: https://github.com/veil/veil/issues')
	...
	async def run(
		...
		from veil.utils import SignalHandler
	...
	async def _generate_rerun_summary(
		...
		from veil.agent.views import RerunSummaryAction
		...
		from veil.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
		...
		try:
			...
			from veil.llm.messages import BaseMessage
	...
	async def _execute_ai_step(
		...
		"""
		from veil.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from veil.llm.messages import SystemMessage, UserMessage
		from veil.utils import sanitize_surrogates
		...
		try:
			from veil.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from veil.agent.variable_detector import detect_variables_in_history
