...
from veil.dom.serializer.clickable_elements import ClickableElementDetector
from veil.dom.serializer.paint_order import PaintOrderRemover
from veil.dom.utils import cap_text_length
...
class DOMTreeSerializer:
	...
	def _create_simplified_tree(self, node: EnhancedDOMTreeNode, depth: int = 0) -> SimplifiedNode | None:
		...
		if node.node_type == NodeType.DOCUMENT_FRAGMENT_NODE:
			...
			attr_type = None
			if self.session_id:
				session_specific_attr = f'data-veil-exclude-{self.session_id}'
			...
			if not exclude_attr:
				exclude_attr = attributes.get('data-veil-exclude')
	...
	def _assign_interactive_indices_and_mark_new_nodes(self, node: SimplifiedNode | None) -> None:
		...
		if not node.excluded_by_parent and not node.ignored_by_paint_order:
			...
			if is_interactive_assign and not node.original_node.snapshot_node:
				...
				logger = logging.getLogger('veil.dom.serializer')
