...
_DEMO_PANEL_SCRIPT = r"""(function () {
  ...
  const EXCLUDE_ATTR = 'data-veil-exclude-' + SESSION_ID;
  const PANEL_ID = 'veil-demo-panel';
  const STYLE_ID = 'veil-demo-panel-style';
  ...
  const TOGGLE_BUTTON_ID = 'veil-demo-toggle';
  ...
  function initializePanel() {
    ...
    if (savedWidth) {
      document.documentElement.style.setProperty('--veil-demo-panel-width', `${savedWidth}px`);
    }
  ...
  function addStyles() {
    ...
    style.textContent = `
      #${PANEL_ID} {
        ...
        top: 0;
        right: 0;
        width: var(--veil-demo-panel-width, 340px);
      ...
      #${PANEL_ID} .veil-logo img {
        height: 36px;
      }

      #${PANEL_ID} .veil-header-actions {
        margin-left: auto;
        display: flex;
        align-items: center;
        gap: 8px;
      ...
      #${PANEL_ID} .veil-close-btn:hover {
        background: rgba(255, 255, 255, 0.15);
        border-color: rgba(255, 255, 255, 0.35);
      }

      #${PANEL_ID} .veil-demo-body {
        flex: 1;
        overflow-y: auto;
      ...
      #${PANEL_ID} .veil-demo-body::-webkit-scrollbar {
        width: 8px;
      }

      #${PANEL_ID} .veil-demo-body::-webkit-scrollbar-thumb {
      ...
      .veil-demo-entry {
        ...
        animation: veil-fade-in 0.25s ease;
        background: #000000;
      }

      .veil-demo-entry:last-child {
      ...
      .veil-markdown-content {
        margin: 0;
        line-height: 1.5;
      }

      .veil-markdown-content p {
        margin: 0 0 8px 0;
      }

      .veil-markdown-content p:last-child {
        margin-bottom: 0;
      }

      .veil-markdown-content h1,
      .veil-markdown-content h2,
      .veil-markdown-content h3 {
        margin: 8px 0 4px 0;
        font-weight: 600;
        color: #f8f9ff;
      }
      ...
      .veil-markdown-content pre code {
      ...
      .veil-markdown-content ul,
      .veil-markdown-content ol {
        margin: 4px 0 4px 16px;
        padding: 0;
      }

      .veil-markdown-content li {
        margin: 2px 0;
      }

      .veil-markdown-content a {
        color: #60a5fa;
        text-decoration: underline;
      }

      .veil-markdown-content a:hover {
        color: #93c5fd;
      }

      .veil-markdown-content strong {
        font-weight: 600;
        color: #f8f9ff;
      }

      .veil-markdown-content em {
        font-style: italic;
      }

      .veil-demo-entry:not(.expanded) .veil-markdown-content {
        max-height: 120px;
        overflow: hidden;
      ...
      .veil-demo-entry.level-info { border-left-color: #60a5fa; }
      .veil-demo-entry.level-action { border-left-color: #34d399; }
      .veil-demo-entry.level-thought { border-left-color: #f97316; }
      .veil-demo-entry.level-warning { border-left-color: #fbbf24; }
      .veil-demo-entry.level-success { border-left-color: #22c55e; }
      .veil-demo-entry.level-error { border-left-color: #f87171; }
  ...
  function buildPanel() {
    ...
    header.className = 'veil-demo-header';
    const title = document.createElement('div');
    title.className = 'veil-logo';
    const logo = document.createElement('img');
    logo.src = 'https://raw.githubusercontent.com/veil/veil/main/static/veil-dark.png';
    logo.alt = 'Browser-use';
    logo.loading = 'lazy';
    title.appendChild(logo);
    const actions = document.createElement('div');
    actions.className = 'veil-header-actions';
    const closeBtn = document.createElement('button');
    closeBtn.type = 'button';
    closeBtn.className = 'veil-close-btn';
  ...
  function adjustLayout() {
    ...
    document.documentElement.style.setProperty('--veil-demo-panel-width', `${width}px`);
    if (state.isOpen) {
  ...
  function hydrateFromStoredMarkup() {
    ...
    try {
      ...
      if (html) {
        ...
        for (const entryNode of state.list.querySelectorAll('.veil-demo-entry')) {
          const toggle = entryNode.querySelector('.veil-entry-toggle');
          if (toggle) {
  ...
  function createEntryNode(entry) {
    ...
    if (messageText.length > 160) {
      ...
      toggle.type = 'button';
      toggle.className = 'veil-entry-toggle';
  ...
  function applyPersistedExpansion(node) {
    ...
    try {
      ...
      if (id && expanded.has(id)) {
        ...
        const toggle = node.querySelector('.veil-entry-toggle');
        if (toggle) {
  ...
  window.addEventListener('veil-log', handleLogEvent);
})();
"""
...
(() => {{
	...
	const event = new CustomEvent('veil-log', {{ detail }});
