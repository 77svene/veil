...
from veil.browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError

# CDP logging is now handled by setup_logging() in logging_config.py
# It automatically sets CDP logs to the same level as veil logs
from veil.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
from veil.browser.events import (
...
from veil.browser.profile import BrowserProfile, ProxySettings
from veil.browser.views import BrowserStateSummary, TabInfo
from veil.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from veil.observability import observe_debug
from veil.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from veil.actor.page import Page
	from veil.browser.demo_mode import DemoMode
	from veil.browser.watchdogs.captcha_watchdog import CaptchaWaitResult
...
class BrowserSession(BaseModel):
	...
	def from_system_chrome(cls, profile_directory: str | None = None, **kwargs: Any) -> Self:
		...
		from veil.skill_cli.utils import find_chrome_executable, get_chrome_profile_path, list_chrome_profiles
	...
	def list_chrome_profiles(cls) -> list[dict[str, str]]:
		...
		from veil.skill_cli.utils import list_chrome_profiles
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'veil.{self}')
		return logging.getLogger(f'veil.{self}')
	...
	def model_post_init(self, __context) -> None:
		...
		from veil.browser.watchdog_base import BaseWatchdog
	...
	async def kill(self) -> None:
		...
		from veil.browser.events import SaveStorageStateEvent
				...
				if self.browser_profile.use_cloud or self.browser_profile.cloud_browser_params is not None:
					...
					try:
						...
						raise CloudBrowserAuthError(
							'Authentication failed for cloud browser service. Set BROWSER_USE_API_KEY environment variable. You can also create an API key at https://cloud.veil.com/new-api-key'
						)
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from veil.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from veil.actor.page import Page as PageActor
	...
	async def attach_all_watchdogs(self) -> None:
		...
		from veil.browser.watchdogs.aboutblank_watchdog import AboutBlankWatchdog
		from veil.browser.watchdogs.captcha_watchdog import CaptchaWatchdog

		# from veil.browser.crash_watchdog import CrashWatchdog
		from veil.browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from veil.browser.watchdogs.dom_watchdog import DOMWatchdog
		from veil.browser.watchdogs.downloads_watchdog import DownloadsWatchdog
		from veil.browser.watchdogs.har_recording_watchdog import HarRecordingWatchdog
		from veil.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from veil.browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from veil.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from veil.browser.watchdogs.recording_watchdog import RecordingWatchdog
		from veil.browser.watchdogs.screenshot_watchdog import ScreenshotWatchdog
		from veil.browser.watchdogs.security_watchdog import SecurityWatchdog
		from veil.browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		try:
			...
			from veil.browser.session_manager import SessionManager
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from veil.browser.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from veil.dom.views import NodeType
	...
	async def remove_highlights(self) -> None:
		...
		try:
			async with asyncio.timeout(3.0):
				...
				script = """
				(function() {
					// Remove all veil highlight elements
					const highlights = document.querySelectorAll('[data-veil-highlight]');
					console.log('Removing', highlights.length, 'veil highlight elements');
					highlights.forEach(el => el.remove());

					// Also remove by ID in case selector missed anything
					const highlightContainer = document.getElementById('veil-debug-highlights');
					...
					const orphanedTooltips = document.querySelectorAll('[data-veil-highlight="tooltip"]');
	...
	async def highlight_interaction_element(self, node: 'EnhancedDOMTreeNode') -> None:
		...
		try:
			...
			(function() {{
				...
				container.setAttribute('data-veil-interaction-highlight', 'true');
	...
	async def highlight_coordinate_click(self, x: int, y: int) -> None:
		...
		try:
			...
			(function() {{
				...
				// Create container
				const container = document.createElement('div');
				container.setAttribute('data-veil-coordinate-highlight', 'true');
				container.style.cssText = `
					position: absolute;
					left: ${{x + scrollX}}px;
					top: ${{y + scrollY}}px;
	...
	async def add_highlights(self, selector_map: dict[int, 'EnhancedDOMTreeNode']) -> None:
		...
		try:
			...
			(function() {{
				...
				const existingContainer = document.getElementById('veil-debug-highlights');
				...
				const strayHighlights = document.querySelectorAll('[data-veil-highlight]');
				...
				const container = document.createElement('div');
				container.id = 'veil-debug-highlights';
				container.setAttribute('data-veil-highlight', 'container');
				...
				interactiveElements.forEach((element, index) => {{
					...
					highlight.setAttribute('data-veil-highlight', 'element');
