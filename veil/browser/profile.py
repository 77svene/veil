...
from veil.browser.cloud.views import CloudBrowserParams
from veil.config import CONFIG
from veil.utils import _log_pretty_path, logger
...
# Using constants from central location in veil.config
...
class BrowserLaunchArgs(BaseModel):
	...
	def set_default_downloads_path(self) -> Self:
		...
		if self.downloads_path is None:
			...
			downloads_path = Path(tempfile.gettempdir()) / f'veil-downloads-{unique_id}'
...
class BrowserLaunchPersistentContextArgs(BrowserLaunchArgs, BrowserContextArgs):
	...
	def validate_user_data_dir(cls, v: str | Path | None) -> str | Path:
		...
		if v is None:
			return tempfile.mkdtemp(prefix='veil-user-data-dir-')
...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	use_cloud: bool = Field(
		default=False,
		description='Use veil cloud browser service instead of local browser',
	)
	...
	proxy: ProxySettings | None = Field(
		default=None,
		description='Proxy settings. Use veil.browser.profile.ProxySettings(server, bypass, username, password)',
	)
	...
	def _copy_profile(self) -> None:
		...
		if 'veil-user-data-dir-' in user_data_str.lower():
		...
		temp_dir = tempfile.mkdtemp(prefix='veil-user-data-dir-')
