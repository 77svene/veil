...
if len(sys.argv) > 1 and sys.argv[1] == 'install':
	...
	if result.returncode == 0:
		...
		print('🚀 Ready to use! Run: uvx veil')
	else:
		print('\n❌ Installation failed')
		sys.exit(1)
	sys.exit(0)
...
if 'init' in sys.argv:
	from veil.init_cmd import INIT_TEMPLATES
	from veil.init_cmd import main as init_main
...
if '--template' in sys.argv:
	...
	output_path = Path(output) if output else Path.cwd() / f'veil_{template}.py'
	...
	try:
		...
		click.echo('\nNext steps:')
		click.echo('  1. Install veil:')
		click.echo('     uv pip install veil')
		click.echo('  2. Set up your API key in .env file or environment:')
		click.echo('     BROWSER_USE_API_KEY=your-key')
		click.echo('     (Get your key at https://cloud.veil.com/new-api-key)')
...
from veil import Agent, Controller
from veil.agent.views import AgentSettings
from veil.browser import BrowserProfile, BrowserSession
from veil.logging_config import addLoggingLevel
from veil.telemetry import CLITelemetryEvent, ProductTelemetry
from veil.utils import get_veil_version
...
try:
	...
	print(
		'⚠️ CLI addon is not installed. Please install it with: `pip install "veil[cli]"` and try again.', file=sys.stderr
	)
	sys.exit(1)
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('veil.'):
		...
		# Configure veil logger and all its sub-loggers
		veil_logger = logging.getLogger('veil')
		veil_logger.propagate = False  # Don't propagate to root logger
		veil_logger.handlers = [log_handler]  # Replace any existing handlers
		veil_logger.setLevel(root.level)

		# Also ensure agent loggers go to the main output
		# Use a wildcard pattern to catch all agent-related loggers
		for logger_name in ['veil.Agent', 'veil.controller', 'veil.agent', 'veil.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'veil.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('veil.on_mount')
	...
	async def action_quit(self) -> None:
		...
		self.exit()
		print('\nTry running tasks on our cloud: https://veil.com')
		...
		with Container(id='main-container'):
			...
			with Container(id='links-panel'):
				with HorizontalGroup(classes='link-row'):
					...
					yield Link('https://veil.com', url='https://veil.com', classes='link-white link-url')
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/veil/awesome-prompts',
						url='https://github.com/veil/awesome-prompts',
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/veil/veil/issues',
						url='https://github.com/veil/veil/issues',
...
async def run_prompt_mode(prompt: str, ctx: click.Context, debug: bool = False):
	"""Run veil in non-interactive mode with a single prompt."""
...
async def textual_interface(config: dict[str, Any]):
	...
	# Prevent veil from setting up logging at import time
	os.environ['BROWSER_USE_SETUP_LOGGING'] = 'false'

	logger = logging.getLogger('veil.startup')
	...
	try:
		...
		try:
			from veil.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from veil.sync.auth import DeviceAuthClient
	...
	try:
		...
		from veil.sync.service import CloudSync
		...
		task_event = CreateAgentTaskEvent(
			...
			llm_model='auth-flow',
			task='🔐 Complete authentication and join the veil community',
			user_id=auth_client.temp_user_id,
			device_id=auth_client.device_id,
			done_output=None,
		...
		try:
			...
			from veil.utils import create_task_with_error_handling
		...
		if success:
			...
			step_event = CreateAgentStepEvent(
				...
				actions=[
					{
						...
						'done': {
							'success': True,
							'text': '⭐ Starred veil/veil repository! Welcome to the community!',
						},
					}
				],
				next_goal='⭐ Star veil GitHub repository to join the community',
				...
				screenshot_url=None,
				url='https://github.com/veil/veil',
			)
			...
			print('   Future veil runs will now sync to the cloud.')
		else:
		...
		if task_id and sync_service:
			try:
				from veil.agent.cloud_events import UpdateAgentTaskEvent
...
@click.option('--output', '-o', type=click.Path(), help='Output file path for template (default: veil_<template>.py)')
...
def main(ctx: click.Context, debug: bool = False, **kwargs):
	...
	Examples:
	  uvx veil --template default
	  uvx veil --template advanced --output my_script.py
	"""
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from veil.mcp.server import main as mcp_main
...
def _run_template_generation(template: str, output: str | None, force: bool):
	...
	if output:
		...
		output_path = Path.cwd() / f'veil_{template}.py'
...
@click.option(
	...
	'-o',
	type=click.Path(),
	help='Output file path (default: veil_<template>.py)',
)
@click.option(
	'--force',
	'-f',
...
def init(
	...
	"""
	Generate a veil template file to get started quickly.
	...
	uvx veil init --template default

	\b
	# Generate advanced template with custom filename
	uvx veil init --template advanced --output my_script.py
